...
describe(CollectionConfig.contractName, function () {
  ...
  it('Check initial data', async function () {
    ...
    expect(await contract.greenlistMintEnabled()).to.equal(false);
  ...
  it('Before any sale', async function () {
    ...
    await expect(contract.connect(whitelistedUser).greenlistMint(1, [], {value: getPrice(SaleType.WHITELIST, 1)})).to.be.revertedWith('The whitelist sale is not enabled!');
    ...
    await expect(contract.connect(holder).greenlistMint(1, [], {value: getPrice(SaleType.WHITELIST, 1)})).to.be.revertedWith('The whitelist sale is not enabled!');
    await expect(contract.connect(owner).mint(1, {value: getPrice(SaleType.WHITELIST, 1)})).to.be.revertedWith('The contract is paused!');
    await expect(contract.connect(owner).greenlistMint(1, [], {value: getPrice(SaleType.WHITELIST, 1)})).to.be.revertedWith('The whitelist sale is not enabled!');

    // The owner should always be able to run mintForAddress
    await (await contract.reserve(1, await owner.getAddress())).wait();
    await (await contract.reserve(1, await whitelistedUser.getAddress())).wait();
    // But not over the maxMintAmountPerTx
    await expect(contract.reserve(
      await (await contract.maxMintAmountPerTx()).add(1),
      await holder.getAddress(),
    )).to.be.revertedWith('Invalid mint amount!');

    // Check balances
    expect(await contract.balanceOf(await owner.getAddress(),1)).to.equal(1);
    expect(await contract.balanceOf(await whitelistedUser.getAddress(),1)).to.equal(1);
    expect(await contract.balanceOf(await holder.getAddress(),1)).to.equal(0);
    expect(await contract.balanceOf(await externalUser.getAddress(),1)).to.equal(0);
  });
  ...
  it('Whitelist sale', async function () {
    ...
    await contract.setGreenlistMintEnabled(true);

    await contract.connect(whitelistedUser).greenlistMint(
      1,
      merkleTree.getHexProof(keccak256(await whitelistedUser.getAddress())),
      {value: getPrice(SaleType.WHITELIST, 1)},
    );
    // Trying to mint twice
    await expect(contract.connect(whitelistedUser).greenlistMint(
      1,
      merkleTree.getHexProof(keccak256(await whitelistedUser.getAddress())),
      {value: getPrice(SaleType.WHITELIST, 1)},
    ...
    await expect(contract.connect(holder).greenlistMint(
      1,
      merkleTree.getHexProof(keccak256(await whitelistedUser.getAddress())),
      {value: getPrice(SaleType.WHITELIST, 1)},
    ...
    await contract.setGreenlistMintEnabled(false);
    await contract.setCost(utils.parseEther(CollectionConfig.preSale.price.toString()));

    // Check balances
    expect(await contract.balanceOf(await owner.getAddress(),1)).to.equal(1);
    expect(await contract.balanceOf(await whitelistedUser.getAddress(),1)).to.equal(2);
    expect(await contract.balanceOf(await holder.getAddress(),1)).to.equal(0);
    expect(await contract.balanceOf(await externalUser.getAddress(),1)).to.equal(0);
  });
  ...
  it('Owner only functions', async function () {
    await expect(contract.connect(externalUser).reserve(1, await externalUser.getAddress())).to.be.revertedWith('Ownable: caller is not the owner');
    ...
    await expect(contract.connect(externalUser).setGreenlistMintEnabled(false)).to.be.revertedWith('Ownable: caller is not the owner');
  ...
  it('Supply checks (long)', async function () {
    ...
    expect(await contract.maxSupply()).to.equal(alreadyMinted);
    ...
    expect(await contract.maxSupply()).to.equal(expectedTotalSupply);
